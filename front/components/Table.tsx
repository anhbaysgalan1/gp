...
function handleWinner(game: GameType | null, sendMessage: ((message: any) => void) | null) {
    if (!game || !sendMessage) {
        return null;
    }
    if (game && game.stage === 1 && game.pots.length !== 0) {
        ...
        if (!winningPlayer) {
            return null; // No winning player found, skip winner handling
        }
        const pot = game.pots[game.pots.length - 1].amount;
        const message = winningPlayer.username + " wins " + pot;
        sendMessage({ action: "log", message: message });
    }
}
...
export default function Table({ players, setPlayers }: tableProps) {
    ...
    useEffect(() => {
        ...
        if (game && game.stage === 1 && game.pots.length !== 0) {
            ...
            handleWinner(game, sendMessage);
            const timer = setTimeout(() => {
                setRevealedPlayers([]);
                if (sendMessage) {
                    sendMessage({ action: "deal" });
                }
            }, 5000);
            return () => {
                clearTimeout(timer);
