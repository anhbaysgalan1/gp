...
import { useTableWebSocket } from '../../contexts/WebSocketContext';
...
function GamePage() {
  ...
  const { sendMessage, isConnected, leaveTable } = useTableWebSocket(typeof tableId === 'string' ? tableId : undefined);
  ...
  const handleLeaveTable = () => {
    if (!tableId || typeof tableId !== 'string' || !isConnected) return;

    try {
      // Send leave-table message via WebSocket
      leaveTable(tableId);
      console.log("Sent leave table request via WebSocket");

      // Navigate to lobby after sending the message
      router.push('/lobby');
    } catch (err) {
